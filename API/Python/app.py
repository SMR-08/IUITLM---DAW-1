...
def api_procesar_nlp_completo():
    """
    Endpoint unificado para procesar texto (tokenizar y etiquetar POS).
    Acepta un parámetro 'idioma' ('es' o 'en') en el JSON.
    ...
    try:
        ...
        idioma = datos.get('idioma', 'es').lower() # Español por defecto
        if idioma not in ['es', 'en']:
            raise ValueError("Idioma no soportado. Usar 'es' o 'en'.")
    ...
    # Procesar el texto según el idioma
    lista_resultado = []
    error_procesamiento = None
    try:
        if idioma == 'es':
            lista_resultado = procesar_texto_spacy(texto_entrada)
            # Verificar si spaCy devolvió error interno
            if lista_resultado and lista_resultado[0][0] == "Error:":
                 error_procesamiento = f"Error interno spaCy: {lista_resultado[0][1]}"
                 lista_resultado = [] # No enviar el error interno al cliente así
        elif idioma == 'en':
            lista_resultado = etiquetar_palabras_nltk(texto_entrada)
             # Verificar si NLTK devolvió error interno
            if lista_resultado and lista_resultado[0][0] == "Error:":
                 error_procesamiento = f"Error interno NLTK: {lista_resultado[0][1]}"
                 lista_resultado = []

        # Comprobar si hubo error durante el procesamiento específico
        if error_procesamiento:
             log.error(f"Error detectado durante procesamiento NLP ({idioma}): {error_procesamiento}")
             return jsonify({'error': f'Error al procesar el texto ({error_procesamiento}). Comprueba los logs del servidor.'}), 500 # Error interno

        # Devolver respuesta exitosa
        log.info(f"Procesamiento NLP ({idioma}) completado, {len(lista_resultado)} tokens/etiquetas generadas.")
        return jsonify({
            'resultado': lista_resultado,
            'mensaje': f'Texto procesado ({idioma.upper()}) correctamente.',
            'idioma_procesado': idioma # Devolver el idioma usado para que JS sepa qué paleta usar
        }), 200
